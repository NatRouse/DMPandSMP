function d = Eval(w, Nt, y0dot, y0, x0, alphax, tau, dt, sigma_or_rho,...
    c_or_alpha, alphay, betay,g, desiredtraj, ShowPlot, OneforDMPtwoforRMP,...
    w1,w2,Nsaddles)

    %evaluates the "cost" of an RMP-based movement and returns "cost"

    %w = array of basis function weights
    %Nt = number of timesteps
    %desiredtraj = desired position at each timestep. array?
    %w1 = initial weights sampled from trajectory
    %w2 = initial trajectory weights, but scaled up/down
    %g = goal position of system
    %Nsaddles = number of basis functions

    goaltol = .005;
    
    %some values that WERE initialized at first, but now get passed
    %ShowPlot = 1;
    ...
    if OneforDMPtwoforRMP == 1
        ...
        sigma = sigma_or_rho;
        c = c_or_alpha;
            %these get passed as arguments to this function

        x = x0;
        if ShowPlot %ShowPlot is an argument
            xall = nan*ones(Nt,1);
        ...
        beta = 1;
        nu = 1.2;
            %some values used for the RMP?? idk where they are in the
            %formulation
        rho = calculaterho_notacycle(alpha*ones(Nsaddles,1), beta*ones(Nsaddles,1), nu*ones(Nsaddles,1));
            %some other function Natasha wrote - FIND THIS!!!
    ...
    %distance away from desired trajectory
        %Clean this up
