...
desiredtraj = [131,82
                ...
                118,204
                114,194];
desiredtraj = [0.8*cos(-pi/2:.1:pi/2),cos(-pi/2:.1:pi/2);
            0.5*sin(-pi/2:.1:pi/2)-0.5,0.3+0.3*sin(-pi/2:.1:pi/2)]';
desiredtraj(:,2) = flip(desiredtraj(:,2));
g = desiredtraj(end,:);
...
%Elongated Heart -----
% t = linspace(-1,1,100);
% desiredtraj = [sin(t).*cos(t).*log10(abs(t));
%     (abs(t).^0.3).*sqrt(cos(t))]';
% g = desiredtraj(end,:);
...
%Spacing
sigma = linspace(log10(.3),log10(.002), bfs);
% sigma = logspace(log10(.3),log10(.002), bfs); %original
sigma(sbfs/2) = sigma(sbfs/2) + 10;
sigma(sbfs/2 - 1) = sigma(sbfs/2 - 1) + 5;
sigma(sbfs/2 + 1) = sigma(sbfs/2 + 1) + 5;

%Passing time
tau = 2*ones(1,sbfs);
% tau((sbfs/2)-2:(sbfs/2)+2) = 3;
tau(sbfs/2) = tau(sbfs/2)*2;
tau(1) = tau(1)*0.75;
tau(end) = tau(end)*0.75;
...
%According to Horchler paper, the steps to achieve preceise desired
%sub-periods for a LV SHC cycle are:
% 1. Choose beta, nu & epsilon-hat
% 2. Solve for alpha using eq. (11), and construct rho matrix
% 3. Simulate eq. (2) to measure mean sub-periods which should
% approximately be the desired sub-periods.
% 4. Perform non-linear compensation by adjusting the noise with eq. (14).

%SMP INITIALIZATION
S = struct('tauy',1,'alphay',4,'betay',1,'tau',tau,'alpha',[],'beta',ones(1,sbfs),...
    'nu',1.2*ones(1,sbfs),'bfs',sbfs,...
    'rho',zeros(sbfs),...
    'a0',[1;0.1;10^-9*ones(sbfs-2,1)],...
    'a',[],...'ep',10^-9*ones(1,sbfs),...
    'ep',10^-6*ones(1,sbfs),...
    'w',zeros(DOF,bfs),...
    'f',zeros(P.Nt,DOF),'y',y0,'ydot',y0dot);

[S.alpha,S.beta,S.nu] = shc_lv_params(S.tau,S.ep,S.beta,S.nu,0);
%Putting in this nonsense loop
% for i = 1:bfs
%     hpaper_alphafun = @(nu,i) 0.5
% % my_alpha
...
%Make rho matrix with "reversed direction"
S.rho = shc_lv_createcycle(S.alpha,S.beta,S.nu,-1);

% %OLD WAY: Parameters for rho matrix
% %This set of parameters does not exactly match the Horchler Toolbox
% alpha_rho = S.alpha;
% beta_rho = S.beta;
% nu_rho = S.nu;
% new_rho = zeros(S.bfs);
...
%     for j = 1:S.bfs
%         if i==j
%             new_rho(i,j) = alpha_rho(i)/beta_rho(i);
%         else
%             if 0 == j-1-i %i == mod(j,S.bfs)+1
%                 new_rho(i,j) = (alpha_rho(i) - alpha_rho(j)/nu_rho(j))./beta_rho(j);
%             else
%                 new_rho(i,j) = (alpha_rho(i) + alpha_rho(j))/beta_rho(j);
%             end
%         end
%     end
% end
% comparison = S.rho==new_rho
...
fdesiredr = S.tauy * desiredydotdot - (S.alphay *...
...
for i = 1:P.Nt
    ...
    ydotdot = (S.alphay*(S.betay*(P.g-S.y(i,:))-S.ydot(i,:))+S.f(i,:))/S.tauy;
    ydotdotsum = rssq(ydotdot) + ydotdotsum;
